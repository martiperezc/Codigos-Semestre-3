...
int buscarProducto(Libros biblioteca[], int n, char nombreBuscado[], int k, char estado[]) {
    validar_strings(nombreBuscado, 50, "Ingrese el titulo del libro que desea encontrar: ");
    for (int i = 0; i < n; i++) {
        if (strcmp(biblioteca[i].titulo, nombreBuscado) == 0){
            k = obtener_entero_valido("Desea actualizar el estado de este libro? Donde: 1)Si \t 0)NO:  ");
            if (k==1){
            validar_strings(estado,50, "A que estado actualizara?:  ");   //Usario ingresa nuevo estado del libro buscado
            strcpy(biblioteca[i].estado, estado);  //Se le asigna el nuevo estado al libro buscado
            return i;
            }
            return i; //Devuelve el indice del producto buscado.
        }
    }
...
int buscarProductoID(Libros biblioteca[], int n, int idbuscado, int k, char estado[]){
    idbuscado = obtener_entero_valido("Ingrese el ID del producto que desea encontrar: ");
    for (int i=0; i<n;i++){
        if((biblioteca[i].id-idbuscado)==0){
            k = obtener_entero_valido("Desea actualizar el estado de este libro? Donde: 1)Si \t 0)NO:  ");
            if (k==1){
            validar_strings(estado,50, "A que estado actualizara?:  ");   //Usario ingresa nuevo estado del libro buscado
            strcpy(biblioteca[i].estado, estado);  //Se le asigna el nuevo estado al libro buscado
...
void imprimir_info(Libros biblioteca[], int indice){
            if (indice == -1){
             printf("Ese producto no se encuentra registrado\n");
            } else{
            printf("Informacion del libro %s\n", biblioteca[indice].titulo); 
            printf("ID del libro: %d\n", biblioteca[indice].id); 
            printf("Autor: %s\n", biblioteca[indice].autor); 
            printf("Year: %d\n", biblioteca[indice].anio); 
